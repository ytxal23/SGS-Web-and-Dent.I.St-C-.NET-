...
function editClass(row_id){
  ...
  var tmname = $("#" + row_id).find(".editable")[3].textContent;
    
  $.post("edit-data_subj.php" , {tid:tid, tcode:tcode, tfname:tfname, tmname:tmname} , function(data){
    $("#result").html(data);
...
function delClass(row_id){
  ...
  $.post("inastud_subj.php" , {tid:tid, tcode:tcode, tfname:tfname, tmname:tmname} , function(data){
    $("#result").html(data);
    $("#" + tid).fadeOut('slow');
  } );
}
          ...
          <div class="row">
            <div class="col-md-8">
              ...
              <h1 class="panel-heading" class="hidden">
                ...
                <p><a style="font-size: 30pt; font-style: italic;" href="home_admin.php"><img src="img/sgslogo.png" style="float: left" width="75px" height="75px" hspace="10" >
                <h1 text-align="center">School Grading System</h1></p></a>             
                <!--h1 class="hidden">
   ...
   echo "ERROR";
          ...
          ?>
              <div id="openModal" class="modalDialog">
                    ...
                    $sid=autoincsubj();
                      echo'
                    <form id="addstud" method ="post" action = "subj_name_check.php">
                        <label>Subject Code:</label><br>
                        <input maxlength="10" pattern="[a-zA-Z0-9]{45}" title="Alphanumeric Characters only" type="text" class="form-style-9 form-textfield-third align-left" placeholder="Subject Code" name="strsubjcode" id="strsubjcode" maxlength=10 onload="autoincsubj()" value="'.$sid.'" required disabled/><br>
                        <label>Subject:</label><br>
                        <input type="text" pattern="[a-zA-Z\s]{3, 45}" title="Letters and spaces only" maxlength=45 class="form-style-9 form-textfield-third align-left" placeholder="Subject Name" name="strsubjname" id="strsubjname" maxlength=45 required/><br>
                        <label>Description:</label><br>
                        <input type="text" pattern="[a-zA-Z\s]{10,100}" title="Letters and spaces only" class="form-style-9 form-textfield-third align-center" placeholder="Description" name="strsubjdesc" id="strsubjdesc" maxlength=100 required/><br><br>
           ...
           <div class="search-box right" style="color: black; width: 17.5%">
            <input type="text" id='subject' autocomplete="off" placeholder="Search subject.." />
            <div class="result">
            </div>
          </div><br>
        ...
        <div class="w3-panel2 w3-blue w3-card-4">
          ...
          <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            <table id = "subjgrid" class="table table-striped table-style-two" cellpadding="0" cellspacing="0">
            <?php include('table_subj.php');?>
            </table>
        </div>
...
<script type="text/javascript">
    ...
    function insertData(){  
           ...
           $.ajax({  
                url:"insert-data_subj.php",  
                method: "POST",  
                 data:{strsubjcode = $('#strsubjcode').text(), strsubjname = $('#strsubjname').text(), strsubjdesc = $('#strsubjdesc').text()},  
                dataType: "json",
...
function setEditable(row_id){
  var tr = document.getElementById(row_id);
  var tr_elements = $("#subj-grid" + row_id).find(".editable");
  ...
  $("#subj-grid" + row_id).find(".editLink").fadeOut('slow' ,function(){$(this).replaceWith(updateLinkHTML).fadeIn()});
...
function editRow(id) {
        ...
        if ( 'undefined' != typeof id ) {
          $.getJSON('maintenace_subj.php#subj-grid=' + id, function(obj) {
            $('#edit-id').val(obj.id);
            $('#subjcode').val(obj.strsubjcode);
            $('#subjname').val(obj.strsubjname);
            $('#subjdesc').val(obj.strsubjdesc);
